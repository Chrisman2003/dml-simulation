...
def generate_single_dataset(n, p, sigma, seed=42):
    ...
    beta[10:] = 0  
    # Sparse signal: only 10 covariates
...
def evaluate_learners(X, Y, n_folds=5):
    ...
    # 4. Random Forest
    rf_pipe = Pipeline([
        ("model", RandomForestRegressor(random_state=42, max_depth=10, min_samples_leaf=5,))
    ])
    rf_params = {
    ...
    # 5. Gradient Boosting
    gb_pipe = Pipeline([("model", GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=2,random_state=42))])
    gb_params = {
...
# =====================================================
# 3. Data Generating Process (Population)
# =====================================================
...
# =====================================================
# 4. Partial Effect Estimation (Estimators)
# =====================================================
...
# =====================================================
# 6. Parameter Simulation with Best Alpha
# =====================================================
def run_parameter_simulation(n, p, seed, best_alpha):
    ...
    """
    rng = np.random.default_rng(seed)
    X, Y = generate_dataset_with_known_beta(n, p, rng, beta0=1.0)
